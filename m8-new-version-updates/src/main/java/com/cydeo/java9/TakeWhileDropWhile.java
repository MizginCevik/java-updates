...
public class TakeWhileDropWhile {
    ...
    //.peek -> it's used for debugging perspective. Basically, we use it for printing debug process on console.
    ...
    //There is same logic. But What's the difference between filter and takeWhile?
    //filter -> even condition is not matching, it still checks all of them
    //takeWhile -> list should be sorted. it doesn't check when condition is false. It limits based on condition.
    //limit -> doesn't accept condition, just value.
    ...
    //dropWhile -> drops all the matching ones.
    //What's difference between dropWhile and skip?
    //skip based on value, dropWhile based on condition
